...
function getZarfBinary(arch, installPath, platform) {
  const binaryURL = setZarfBinaryUrl(platform, arch);
  core.info(`Downloading the zarf binary from ${ binaryURL }...`);
  const pathToBinary = tc.downloadTool(binaryURL, installPath);
...
function cacheZarfBinary(zarfBinary, version) {
  core.info("Caching the zarf binary...");
  const binaryFile = os.platform().startsWith("win") ? "zarf.exe" : "zarf";
  const toolName = "zarf";
  const binCachedPath = tc.cacheFile(zarfBinary, binaryFile, toolName, version);
...
function getZarfInitPackage(initPackagePath, tarball, version) {
  ...
  const pathToInitPackage = tc.downloadTool(initPackageURL, initPackagePath);
